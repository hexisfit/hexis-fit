...
export default async function handler(req: Request) {
  ...
  if (url.pathname.endsWith("/stats")) {
    ...
    if (!db) return new Response(JSON.stringify({ uploaded: false }), { headers: H });
  ...
  if (req.method === "POST") {
    try {
      ...
      if (!data.recipes || !data.ingredientNames || !data.menu28) {
        return new Response(JSON.stringify({ error: "Invalid database format" }), { status: 400, headers: H });
      }
      await put("recipes/database.json", JSON.stringify(data), { access: "public", addRandomSuffix: false, token: process.env.BLOB_READ_WRITE_TOKEN });
      return new Response(JSON.stringify({ ok: true, recipes: Object.keys(data.recipes).length, menu: data.menu28.length }), { headers: H });
    } catch (e: any) {
  ...
  if (!db) return new Response(JSON.stringify({ error: "No recipe database uploaded" }), { status: 404, headers: H });
